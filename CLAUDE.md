...
- Create a new branch for each feature or logical unit of work.
- Write clear, descriptive commit messages. Prefix with the area: `feat(chat):`, `fix(dashboard):`, `chore(deps):`, `refactor(auth):`, `test(ideas):`.
- Commit often. Small, focused commits are easier to review and revert.
- Do not commit secrets, `.env` files, or `node_modules/`.
- Run `pnpm check` before committing (Husky does this automatically on staged files).
